...
public class ChessMatch {
	...
	private void validateSourcePosition(Position position) {
		...
		if (currentPlayer != ((ChessPiece)board.piece(position)).getColor()) {
			throw new ChessException("Essa peça pertence ao adversario!");
		}
		if (!board.piece(position).isThereAnyPossibleMove()) {
			throw new ChessException("Não existem movimentos possiveis para a peça selecionada!");
		}
	}
	
	private void validateTargetPosition(Position source, Position target) {
		if (!board.piece(source).possibleMove(target)) {
			throw new ChessException("A peça selecionada não pode ser mover para posição selecionada!");
		}
	}
	...
	private ChessPiece king(Color color) {
		...
		throw new IllegalStateException("Rei nao encontrado!");
	}
